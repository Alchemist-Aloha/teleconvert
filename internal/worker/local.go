...
func (l *LocalWorker) StartCommand(ctx context.Context, command, pidFile, exitFile, stderrLog string) (int, error) {
	...
	// Capture both output streams. HandBrakeCLI builds and user-provided
	// wrapper scripts do not consistently emit progress on stderr.
	wrapped := fmt.Sprintf("set -e; rm -f %s %s; nohup sh -c %s >%s 2>&1 </dev/null & pid=$!; echo $pid > %s",
		shellQuote(pidFile),
		shellQuote(exitFile),
